...
R_HOME_DIR=
if test "${R_HOME_DIR}" = "/tmp/R_local_wei402/lib64/R"; then
   case "linux-gnu" in
   linux*)
     run_arch=`uname -m`
     case "$run_arch" in
     ...
     if [ -x "/tmp/R_local_wei402/${libnn}/R/bin/exec/R" ]; then
        R_HOME_DIR="/tmp/R_local_wei402/${libnn}/R"
     elif [ -x "/tmp/R_local_wei402/${libnn_fallback}/R/bin/exec/R" ]; then
        R_HOME_DIR="/tmp/R_local_wei402/${libnn_fallback}/R"
